...
import com.example.demo.model.CatCollegeDto;
import com.example.demo.model.*;
import com.example.demo.repository.CatCollegeRepository;
...
@Service
public class CatCollegeService {

    @Autowired
    private CatCollegeRepository repo;

    @Autowired
    private UserProfileRepository userProfileRepo;

    public List<CatCollegeDto> getEligibleColleges(CollegeFilterRequest req) {

        List<CatCollege> allColleges = repo.findAll();
        List<CatCollegeDto> eligibleColleges = new ArrayList<>();

        for (CatCollege college : allColleges) {
            List<CutoffCat> cutoffs = college.getCutoffs();
            List<String> eligibleBranches = new ArrayList<>();

            for (String requestedBranch : req.getSelectedBranches()) {
                Optional<CutoffCat> matchingCutoff = cutoffs.stream()
            ...
            if (!eligibleBranches.isEmpty()) {
                CatCollegeDto dto = new CatCollegeDto();
    ...
    private Double getCategoryCutoffValue(CutoffCat cutoff, String category) {
