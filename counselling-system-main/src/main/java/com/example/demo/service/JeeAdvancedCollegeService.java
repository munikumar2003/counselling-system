...
import com.example.demo.model.JeeAdvancedCollegeDto;
import com.example.demo.model.*;
import com.example.demo.repository.JeeAdvancedCollegeRepository;
...
@Service
public class JeeAdvancedCollegeService {

    @Autowired
    private JeeAdvancedCollegeRepository repo;

    @Autowired
    private UserProfileRepository userProfileRepo;

    public List<JeeAdvancedCollegeDto> getEligibleColleges(CollegeFilterRequest req) {
        ...
        List<JeeAdvancedCollege> allColleges = repo.findAll();
        List<JeeAdvancedCollegeDto> eligibleColleges = new ArrayList<>();

        for (JeeAdvancedCollege college : allColleges) {
            List<CutoffAdvanced> cutoffs = college.getCutoffs();
            List<String> eligibleBranches = new ArrayList<>();

            //List<String> eligibleBranches = new ArrayList<>();

            for (String requestedBranch : req.getSelectedBranches()) {
                Optional<CutoffAdvanced> matchingCutoff = cutoffs.stream()
            ...
            if (!eligibleBranches.isEmpty()) {
                JeeAdvancedCollegeDto dto = new JeeAdvancedCollegeDto();
                ...
                List<String> highlightStrings = college.getHighlights().stream()
                        .map(HighlightAdvanced::getHighlight)
                        .collect(Collectors.toList());
    ...
    private Double getCategoryCutoffValue(CutoffAdvanced cutoff, String category) {
